...
int main(int argc, char **argv)
{
    ...
    cudaMemcpyToSymbol(d_hPos, hPos, sizeof(vector3) * NUMENTITIES);
    cudaMemcpyToSymbol(d_hVel, hVel, sizeof(vector3) * NUMENTITIES);
    cudaMemcpyToSymbol(mass, mass, sizeof(double) * NUMENTITIES);
    ...
    {
        compute<<<blocks, threads>>>(d_hVel, d_hPos, mass);  // Corrected function call
        cudaDeviceSynchronize();  
    }

    cudaMemcpyFromSymbol(hPos, d_hPos, sizeof(vector3) * NUMENTITIES);
    cudaMemcpyFromSymbol(hVel, d_hVel, sizeof(vector3) * NUMENTITIES);
    cudaMemcpyFromSymbol(mass, mass, sizeof(double) * NUMENTITIES);
